...
class YoloTest(object):
    def __init__(self):
        ...
        with tf.name_scope('input'):
            self.input_data = tf.placeholder(
                tf.float32, [None, None, None, 3], name='input_data')
            self.defog_A = tf.placeholder(
                tf.float32, [None, 3], name='defog_A')
            self.IcA = tf.placeholder(
                tf.float32, [None, None, None, 1], name='IcA')
            self.trainable = tf.placeholder(dtype=tf.bool,    name='trainable')
            self.input_data_clean = tf.placeholder(
                tf.float32, [None, None, None, 3], name='input_data')
    ...
    def predict(self, image, image_name):
        ...
        if self.isp_flag:
            ...
            pred_sbbox, pred_mbbox, pred_lbbox, image_isped, isp_param, filter_imgs_series = self.sess.run(
                [self.pred_sbbox, self.pred_mbbox, self.pred_lbbox,
                    self.image_isped, self.isp_params, self.filter_imgs_series],
                feed_dict={
                    # image_data*np.exp(lowlight_param*np.log(2)),
            ...
            pred_sbbox, pred_mbbox, pred_lbbox, image_isped, isp_param = self.sess.run(
                [self.pred_sbbox, self.pred_mbbox, self.pred_lbbox,
                    self.image_isped, self.isp_params],
                feed_dict={
                    # image_data*np.exp(lowlight_param*np.log(2)),
                    self.input_data: image_data,
                    self.trainable: False
        ...
        if self.isp_flag:
            ...
            image_isped = utils.image_unpreporcess(
                image_isped[0, ...], [org_h, org_w])
    ...
    def evaluate(self):
        ...
        with open(self.annotation_path, 'r') as annotation_file:
            for num, line in enumerate(annotation_file):
                ...
                ground_truth_path = os.path.join(
                    ground_truth_dir_path, str(num) + '.txt')
                ...
                predict_result_path = os.path.join(
                    predicted_dir_path, str(num) + '.txt')
                t1 = time.time()
                bboxes_pr, image_isped, time_one_img = self.predict(
                    image, image_name)
                num_img += 1
                ...
                if self.write_image:
                    if self.isp_flag:
                        image = utils.draw_bbox(
                            image_isped, bboxes_pr, self.classes, show_label=self.show_label)
                    else:
                        image = utils.draw_bbox(
                            image_isped, bboxes_pr, self.classes, show_label=self.show_label)
        ...
        print('validation time:%s, total_proce_time:%s, num_img:%s, aver_time:%s' % (
            time_total, time_total_cnn_process_img, num_img, time_total_cnn_process_img / num_img))
