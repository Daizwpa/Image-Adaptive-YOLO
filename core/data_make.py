...
# '/data/vdd/liuwenyu/data_vocfog/train/JPEGImages/'
    ...
    for i in range(10):
        ...
        center = (row // 2, col // 2)
        #img_name = '.\\data\\vdd\\liuwenyu\\data_vocfog\\train\\JPEGImages\\' + image_name + '_' + ("%.2f" % beta) + '.' + image_name_index
        img_name = '.\\data\\vdd\\liuwenyu\\data_vocfog\\val\\JPEGImages\\' + image_name + '_' + ("%.2f"%beta) + '.' + image_name_index
        if os.path.exists(img_name):
            print(img_name + " is already available ")
            continue
        ...
        print(cv2.imwrite(img_name, img_f))
