...
def convert_voc_annotation(data_path, data_type, anno_path, use_difficult_bbox=False):
    ...
    with open(anno_path, 'a') as f:
        for image_ind in image_inds:
            splited_image_ind = image_ind.split()
            image_path = os.path.join(
                data_path, 'JPEGImages', splited_image_ind[0] + '.jpg')
            annotation = image_path
            label_path = os.path.join(
                data_path, 'Annotations', splited_image_ind[0] + '.xml')
...
if __name__ == '__main__':
    ...
    num1 = convert_voc_annotation(os.path.join(
        flags.data_path, 'train/VOCdevkit/VOC2007'), 'train_test', flags.train_annotation, False)
    num2 = convert_voc_annotation(os.path.join(
        flags.data_path, 'train/VOCdevkit/VOC2012'), 'trainval', flags.train_annotation, False)
    num3 = convert_voc_annotation(os.path.join(
        flags.data_path, 'test/VOCdevkit/VOC2007'),  'test', flags.test_annotation, False)
